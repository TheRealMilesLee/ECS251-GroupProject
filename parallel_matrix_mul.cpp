...
double run_benchmark(const string& method_name, 
                    function<void(vector<vector<int>>&, vector<vector<int>>&, 
                                 vector<vector<int>>&, size_t)> method,
                    MatrixBenchMark& benchmark,
                    vector<vector<int>>& src1, vector<vector<int>>& src2,
                    vector<vector<int>>& dst, size_t block_size) {
  ...
  cout << left << setw(25) << method_name << ": " 
       << fixed << setprecision(6) << seconds << " seconds (" 
...
int main(int argc, char* argv[]) {
  ...
  results.push_back({"Single Thread", 
  ...
  cout << string(80, '-') << endl;
  cout << "Fastest implementation: " << fastest->first 
       << " (" << fixed << setprecision(6) << fastest->second << " seconds)" << endl;
  cout << string(80, '=') << endl;
