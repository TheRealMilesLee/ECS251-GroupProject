...
for matrix_size in 1024 4096 8192; do
  ...
  run_perf() {
    ...
    sudo perf stat -r 10 -e $events -- $cmd $matrix_size 2>&1 | tee Results/$matrix_size/perfStats_${name}_$timestamp.txt
  }
  ...
  for variant in async fifo lifo tbb standard openBLAS openMP; do
    if [ "$variant" != "openBLAS" ]; then
      diff -u <(sort matrix_mul_single.txt) <(sort parallel_matrix_mul_${variant}.txt) |
        grep -E "^[+|-]" | grep -v "+++" >Results/$matrix_size/diff_parallel_${variant}_$(date +"%Y%m%d%H%M%S").txt
      if [ $(wc -c <Results/$matrix_size/diff_parallel_${variant}_*.txt) -eq 0 ]; then
        echo -e "\e[32mValidation Passed for $variant\e[0m"
        rm -rf Results/$matrix_size/diff_parallel_${variant}_*.txt
      else
        echo -e "\e[31mValidation Failed for $variant\e[0m"
      fi
    else
      diff -u <(sort matrix_mul_double.txt) <(sort parallel_matrix_mul_${variant}.txt) |
        grep -E "^[+|-]" | grep -v "+++" >Results/$matrix_size/diff_${variant}_$(date +"%Y%m%d%H%M%S").txt
      if [ $(wc -c <Results/$matrix_size/diff_${variant}_*.txt) -eq 0 ]; then
        echo -e "\e[32mValidation Passed for $variant\e[0m"
        rm -rf Results/$matrix_size/diff_${variant}_*.txt
      else
        echo -e "\e[31mValidation Failed for $variant\e[0m"
      fi
    fi
