...
for variant in async fifo lifo tbb standard openBLAS openMP; do
  if [ "$variant" != "openBLAS" ]; then
    diff -u <(sort matrix_mul_single.txt) <(sort parallel_matrix_mul_${variant}.txt) | grep -E "^[+|-]" | grep -v "+++" >Results/diff_parallel_${variant}_$(date +"%Y%m%d%H%M%S").txt
    ...
    diff -u <(sort matrix_mul_double.txt) <(sort parallel_matrix_mul_${variant}.txt) | grep -E "^[+|-]" | grep -v "+++" >Results/diff_${variant}_$(date +"%Y%m%d%H%M%S").txt
